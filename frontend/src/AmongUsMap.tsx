...
const StyledTask = styled.div<{ bg: string }>`
...
const Task = ({ task }: AmongusTaskProps) => {
  const location_id_to_name: Record<number, string> = {
    0: "Bioscoop",
    1: "Plasdouche",
    2: "Keuken",
    3: "Woonkamer",
  ...
  return (
    <StyledTask bg={task.finished ? "green" : "#ecf0f1"}>
      <div style={{ fontWeight: 600 }}>{task.task_name}</div>
      <div style={{ fontSize: 14 }}>{location_id_to_name[task.location]}</div>
    </StyledTask>
  );
};

export const AmongUsMap = ({ tasks }: AmongUsMapProps) => {
  let rendered_tasks = tasks?.map((x) => <Task task={x} />);
  console.log({ tasks, rendered_tasks });
  return (
    <div style={{ padding: "2rem 1rem" }}>
      <h2 style={{ fontSize: 32, marginTop: 0 }}>Tasks</h2>
      <div style={{ fontSize: 18 }}>{rendered_tasks}</div>
    </div>
  );
